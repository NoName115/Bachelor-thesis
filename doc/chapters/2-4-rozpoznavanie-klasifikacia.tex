...
Tento vyšší-dimenzionálny priestor je tzv. transformovaný priestor príznakov (angl. \textit{transformed feature space}), ako opak ku vstupnému priestoru (angl. \textit{input space}) obsahujúcemu trénovacie dáta \cite{prop:SupervisedMachineLearning}.
...
Takže jadrá(angl. \textit{kernels}) sú špeciálnou triedou funkcií, ktoré dovoľujú, aby sa vnútorné produkty vypočítali priamo vo funkčnom priestore (angl. \textit{feature space}) bez toho, aby sa vykonalo vyššie popísané mapovanie.
